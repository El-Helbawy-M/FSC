...
import '../views/language_selector_page.dart';
...
class SplashViewModel extends MyBaseViewModel {
  ...
  loadNextPage() async {
    ...
    if (AuthServices.firstTimeOnApp()) {
      //choose language
      // Navigator.push(viewContext, MaterialPageRoute(builder: ((context) => LanguageSelectorScreen())));
      await viewContext.navigator.pushAndRemoveUntil(MaterialPageRoute(builder: ((context) => LanguageSelectorScreen())), (route) => false);
      // await showModalBottomSheet(
      //   context: viewContext,
      //   builder: (context) {
      //     return AppLanguageSelector(
